...
if (!existsSync(claudeMd)) {
  ...
  if (!body.includes(name)) {
    fail('CLAUDE.md', `ne cite pas le document canonique (${name}). Une session neuve ne le trouvera pas.`);
  }
}
...
if (errors.length > 0) {
  ...
  console.error('Un gate rouge = documentation a corriger. Ne jamais assouplir la regle pour passer.\n');
  process.exit(1);
}
